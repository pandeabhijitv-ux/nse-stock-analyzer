...
export default function GoldSilverScreen({ onBack }) {
  ...
  const goldData = {
    ...
    symbol: '🥇',
    color: ['#f59e0b', '#d97706'],
    price: liveData?.gold?.price || '71,500',
    unit: '₹/10g (24K)',
    change: liveData?.gold?.change || '+2.3%',
    trend: liveData?.gold?.trend || 'BULLISH',
    planet: 'Shukra (Venus)',
    planetIcon: '♀',
  ...
  const silverData = {
    ...
    symbol: '🥈',
    color: ['#94a3b8', '#64748b'],
    price: liveData?.silver?.price || '82,500',
    unit: '₹/kg',
    change: liveData?.silver?.change || '+1.8%',
    trend: liveData?.silver?.trend || 'BULLISH',
    planet: 'Chandra (Moon)',
    planetIcon: '☽',
